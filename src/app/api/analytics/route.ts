...
export async function GET(request: NextRequest) {
  ...
  const totalDistanceM = runs.reduce((sum, r) => sum + r.totalDistanceM, 0);
  const totalDurationS = runs.reduce((sum, r) => sum + r.totalDurationS, 0);
  ...
  const avgPaceSecsPerKm =
    runsWithPace.length > 0
      ? runsWithPace.reduce((sum, r) => sum + (r.avgPaceSPerKm ?? 0), 0) /
        runsWithPace.length
      : null;
